...
draw_heatmap_g0 <- function(g0_idx){
  ...
  # g1_sets <- vapply(rownames(compar), function(x){get_set_idx(x, g1)}, c(1))
  g0_sets <- vapply(rownames(compar), function(x){get_set_idx(x, g0)}, c(1))
  
  # g1_sets <- unique(g1_sets)
  # g0_sets <- unique(g0_sets)
  # g1_colors <- sample(col_list, size = length(unique(g1_sets)))
  g0_colors <- sample(col_list, size = length(unique(g0_sets)))
  
  # for (set in 1:length(unique(g1_sets))){
  #   row_cols[which(g1_sets == unique(g1_sets)[set])] <- g1_colors[set]
  # }
  ...
  #g0_gr0_compar(C, g0[[898]], gr0[[451]])
  # compar2 <- compar[nrow(compar):1,]
  heatmap.2(compar,Rowv=FALSE,Colv=FALSE, RowSideColors = col_cols, ColSideColors = col_cols, col=rev(bluered(256)), 
...
draw_heatmap_g1 <- function(g1_idx){
  ...
  for (set in 1:length(unique(g1_sets))){
    col_cols[which(g1_sets == unique(g1_sets)[set])] <- g1_colors[set]
  }
  for (set in 1:length(unique(g0_sets))){
    row_cols[which(g0_sets == unique(g0_sets)[set])] <- g0_colors[set]
  }
